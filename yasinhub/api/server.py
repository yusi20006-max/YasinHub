...
class YasinHubHandler(BaseHTTPRequestHandler):
    ...
    def handle_control(self, clean_path: str) -> bool:
        ...
        if clean_path.startswith("/api/control/"):
            ...
            if project is None:
                self.send_json({"service": service, "action": action, "success": False,
                                "error": "service not found", "status": "UNKNOWN", "pid": None}, status=404)
                return True
            ...
            if action == "start":
                ...
                self.send_json({"service": service, "action": action, "success": False,
                                "error": "unknown action", "status": "UNKNOWN", "pid": None}, status=400)
                return True
    ...
    def do_POST(self):
        ...
        if handle_control_api_routes(clean_path, "POST", self.path, getattr(self, "headers", {}),
                                     getattr(self, "rfile", None), self.send_json):
            return
        if self.handle_control(clean_path):
            return
        from .integration_routes import handle_integration_routes
        if handle_integration_routes(clean_path, "POST", self.path, getattr(self, "headers", {}),
                                     getattr(self, "rfile", None), self.send_json):
            return
        from .observer_routes import handle_execution_observer
        if handle_execution_observer(clean_path, "POST", self.path, getattr(self, "headers", {}),
                                     getattr(self, "rfile", None), self.send_json):
            return
        from .slack_routes import handle_slack_routes
        if handle_slack_routes(clean_path, "POST", self.path, getattr(self, "headers", {}),
                               getattr(self, "rfile", None), self.send_json):
            return
    ...
    def do_GET(self):
        ...
        from .control_routes import handle_control_api_routes
        if handle_control_api_routes(clean_path, "GET", self.path, getattr(self, "headers", {}),
                                     getattr(self, "rfile", None), self.send_json):
            return
        if self.handle_control(clean_path):
            return
        from .integration_routes import handle_integration_routes
        if handle_integration_routes(clean_path, "GET", self.path, getattr(self, "headers", {}),
                                     getattr(self, "rfile", None), self.send_json):
            return
        from .observer_routes import handle_execution_observer
        if handle_execution_observer(clean_path, "GET", self.path, getattr(self, "headers", {}),
                                     getattr(self, "rfile", None), self.send_json):
            return
        from .slack_routes import handle_slack_routes
        if handle_slack_routes(clean_path, "GET", self.path, getattr(self, "headers", {}),
                               getattr(self, "rfile", None), self.send_json):
            return
        ...
        if clean_path == "/api/dashboard":
            ...
            summary = {"total_projects": len(reports), "running": 0, "success": 0, "failed": 0,
                       "unknown": 0, "total_posts": 0, "published_posts": 0, "pending_posts": 0}
            projects = []
            for r in reports:
                if r.health_state == "RUNNING": summary["running"] += 1
                elif r.health_state == "SUCCESS": summary["success"] += 1
                elif r.health_state == "FAILED": summary["failed"] += 1
                ...
                projects.append({"name": r.name, "status": r.health_state, "last_run": r.last_run,
                                 "success": r.last_success, "message": r.last_message, "metrics": r.metrics,
                                 "db_stats": r.db_stats, "health": r.health})
            self.send_json({"ecosystem": "Yasin", "dashboard": summary, "projects": projects})
            return

        if clean_path == "/api/status":
            reports = build_report()
            self.send_json({"ecosystem": "Yasin", "projects": [status_project_payload(r) for r in reports]})
            return
        ...
        if clean_path == "/api/services":
            ...
            for p in default_registry():
                ...
                services.append({"name": p.name, "description": p.description, "path": p.path,
                                 "enabled": enabled, "controls": ["start", "stop", "restart"] if enabled else []})
        ...
        if clean_path.startswith("/api/logs/"):
            ...
            try: max_lines = int(query_params.get("lines", ["100"])[0])
            except ValueError: max_lines = 100
            ...
            all_lines = log_file.read_text(encoding="utf-8", errors="ignore").splitlines() if log_file.exists() else []
            if filter_term: all_lines = [line for line in all_lines if filter_term.lower() in line.lower()]
        ...
        if clean_path.startswith("/api/metrics/"):
            ...
            if project is None:
                ...
                return
            data = {"service": service, "status": "UNKNOWN", "pid": None, "cpu": 0, "memory_mb": 0,
                    "uptime": None, "metrics": {}, "db_stats": {}}
            saved_pid = read_pid(service)
            if saved_pid and is_pid_alive(saved_pid): data["pid"] = saved_pid
            try:
        ...
        if clean_path == "/api/events":
            ...
            try: limit = int(limit_str) if limit_str is not None else 50
            except ValueError: limit = 50
            from ..events_engine import parse_events_from_logs, filter_events
            filtered = filter_events(parse_events_from_logs(), service=service, event_type=event_type,
                                     severity=severity, level=level, limit=limit)
        ...
        if clean_path == "/dashboard":
            ...
            if query: redirect_target += f"?{query}"
        ...
        if clean_path.startswith("/dashboard/"):
            ...
            if not relative_path_str or relative_path_str == "/": relative_path_str = "index.html"
            ...
            if file_path.is_relative_to(dashboard_root) and file_path.exists() and file_path.is_file():
                ...
                if file_path.suffix == ".html": content_type = "text/html; charset=utf-8"
...
def run(host="0.0.0.0", port=8000):
    server = HTTPServer((host, port), YasinHubHandler)
    print(f"YasinHub API running on {host}:{port}")
