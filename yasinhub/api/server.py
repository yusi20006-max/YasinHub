...
class YasinHubHandler(BaseHTTPRequestHandler):
    ...
    def handle_control(self, clean_path: str) -> bool:
        ...
        if clean_path.startswith("/api/control/"):
            ...
            if action == "start":
                ...
                return True
            self.send_json({"service": service, "action": action, "success": result})
            return True
        return False
    ...
    def do_POST(self):
        ...
        if handle_control_api_routes(clean_path, "POST", self.path, getattr(self, "headers", {}), getattr(self, "rfile", None), self.send_json):
            return
        if self.handle_control(clean_path):
            return
        from .integration_routes import handle_integration_routes
        if handle_integration_routes(clean_path, "POST", self.path, getattr(self, "headers", {}), getattr(self, "rfile", None), self.send_json):
            return
        from .observer_routes import handle_execution_observer
        if handle_execution_observer(clean_path, "POST", self.path, getattr(self, "headers", {}), getattr(self, "rfile", None), self.send_json):
            return
        from .slack_routes import handle_slack_routes
        if handle_slack_routes(clean_path, "POST", self.path, getattr(self, "headers", {}), getattr(self, "rfile", None), self.send_json):
            return
    ...
    def do_GET(self):
        ...
        clean_path = parsed_url.path
        from .control_routes import handle_control_api_routes
        if handle_control_api_routes(clean_path, "GET", self.path, getattr(self, "headers", {}), getattr(self, "rfile", None), self.send_json):
            return
        if self.handle_control(clean_path):
            return
        from .integration_routes import handle_integration_routes
        if handle_integration_routes(clean_path, "GET", self.path, getattr(self, "headers", {}), getattr(self, "rfile", None), self.send_json):
            return
        from .observer_routes import handle_execution_observer
        if handle_execution_observer(clean_path, "GET", self.path, getattr(self, "headers", {}), getattr(self, "rfile", None), self.send_json):
            return
        from .slack_routes import handle_slack_routes
        ...
        if clean_path.startswith("/api/metrics/"):
            ...
            if project is None:
                self.send_json({"success": False, "error": "service not found"})
                return
            data = {"service": service, "status": "UNKNOWN", "pid": None, "cpu": 0, "memory_mb": 0, "uptime": None, "metrics": {}, "db_stats": {}}
